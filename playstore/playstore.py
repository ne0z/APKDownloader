...
class Playstore(object):
    ...
    def __init__(self, credential: dict = {"USERNAME":"","PASSWORD":"","ANDROID_ID":"","LANG_CODE":"en_US","LANG":"us"}):
        self.logger = logging.getLogger(f"{__name__}.{self.__class__.__name__}")
        self.configuration = credential
        try:
            self.android_id: str = credential["ANDROID_ID"]

            self.email: str = credential["USERNAME"]
            self.encrypted_password: bytes = EncryptedCredentials(
                credential["USERNAME"], credential["PASSWORD"]
            ).get_encrypted_credentials()

            self.lang_code: str = credential["LANG_CODE"]
            self.lang: str = credential["LANG"]
